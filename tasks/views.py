...
class End(LoginRequiredMixin, View):
    login_url = '/login'
    ...
    def get(self, _, pk):
        try:
            ...
            return HttpResponse(e, status=500)
        if task.started:
            timekeeper = TimeKeeper.objects.get(task=task, start__isnull=False, end__isnull=True)
            timekeeper.end = timezone.now()
            try:
                timekeeper.save()
            except Exception as e:
                return HttpResponse(e, status=500)
    ...
    # def get_queryset(self):
    #     if self.task.ended and not self.task.started:
    #         return HttpResponse(),
    #     task = self.model.objects.filter(task=task.started)
    #     return task
